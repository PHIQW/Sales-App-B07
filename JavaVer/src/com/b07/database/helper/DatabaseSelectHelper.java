...
public class DatabaseSelectHelper extends DatabaseSelector {
    ...
    while (results.next()) {
      ...
      log.updateMap(results.getInt("SALEID"),
          new SaleImpl(results.getInt("SALEID"), getSaleById(results.getInt("SALEID")).getUser(),
              (new BigDecimal(getItem(results.getInt("ITEMID")).getPrice().toString())
                  .multiply(new BigDecimal(new Integer(results.getInt("QUANTITY")).toString())))
                      .setScale(2),
              saleItems));
    }
    results.close();
  ...
  private static Account getAuthenticatedAccountDetails(int accountId, User user)
    ...
    Customer dummy =
        new Customer(user.getId(), user.getName(), user.getAge(), user.getAddress(), true);
    while (results.next()) {
   ...
   */
  public static List<Account> getUserActiveAccounts(User user)
      throws SQLException, InputException, ItemNotFoundException, UserNotFoundException {
   ...
   * @throws UserNotFoundException user not found in db
   */
  public static List<Integer> getUserAccountIds(User user)
      throws SQLException, InputException, ItemNotFoundException, UserNotFoundException {
